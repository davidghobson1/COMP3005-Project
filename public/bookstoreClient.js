...
function getQueryParameters(){
	...
	let searchWords = encodeURIComponent(document.getElementById("searchbar").value.trim());
	let category = document.getElementById("searchCategory").value;

	let path = '/store/books?category=' + category + '&searchwords=' + searchWords;
...
function updateBasket(b){
	...
	xmlreq.open("POST", "/store/basket", true);
...
function viewBasket(){
	...
	xmlreq.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200){
			window.location.replace("/store/viewBasket");
		}
	};
	xmlreq.open("GET", "/store/viewBasket", true);
	xmlreq.setRequestHeader("Accept", "text/html");
	xmlreq.send();
}
...
function login(){
	...
	let path = '/store/login?username=' + username + '&password=' + password;
...
function submitOrder(){
	...
	xmlreq.open("POST", "/store/placeOrder", true);
	xmlreq.setRequestHeader("Content-Type", "application/json");
...
function trackOrderLogin(){
	...
	let path = '/store/trackMyOrder?username=' + username + '&password=' + password + '&ordernumber=' + order_number;
...
function addBook(){
	...
	if(confirm("Finish adding the book?\nAll book information is correct?")){
		...
		book["num_pages"] = document.getElementById("num_pages").value;
		...
		xmlreq.open("POST", "/owners/insertBook", true);
		xmlreq.setRequestHeader("Content-Type", "application/json");
...
function removeBook(){
	...
	if(confirm("Confirm you want to delete these books from the inventory?")){
		...
		xmlreq.onreadystatechange = function(){
			...
			if(this.status == 200){
				window.location.href = "/owners/updateInventory";
			}
		};
		xmlreq.open("DELETE", "/owners/removeBooks", true);
		xmlreq.setRequestHeader("Content-Type", "application/json");
...
function getSalesReport(){
	...
	let path = '/owners/salesReport?category=' + category;
